...
def updateStatistics():
	...
	####################
	dbCursor.execute('''DROP TABLE IF EXISTS INDOOR_RANKING_LAPTIME;''')
	dbCursor.execute('''CREATE TABLE INDOOR_RANKING_LAPTIME AS
	SELECT
		kartNumber,
		driverName,
	...
	WHERE driverClass = 'INDOOR'
		...
		AND raceId IN LAST_RACES
	GROUP BY kartNumber
	ORDER BY BEST_LAP;''')
	####################
	dbCursor.execute('''DROP TABLE IF EXISTS PAROLIN_RANKING_LAPTIME;''')
	dbCursor.execute('''CREATE TABLE PAROLIN_RANKING_LAPTIME AS
	SELECT
		kartNumber,
		driverName,
		MIN(bestLapTime) AS 'BEST_LAP',
	...
	;''')
	####################
	dbCursor.execute('''DROP TABLE IF EXISTS TRACK_RANKING_GERAL;''')
	dbCursor.execute('''CREATE TABLE TRACK_RANKING_GERAL AS
	SELECT
		trackConfig,
		driverName,
		driverClass,
	...
	dbCursor.execute('''CREATE TEMPORARY TABLE IF NOT EXISTS TEMP_INDOOR_RANKING_LAPTIME AS SELECT *,(SELECT COUNT(*) FROM INDOOR_RANKING_LAPTIME B WHERE A.BEST_LAP>=B.BEST_LAP) AS RANK_LAPTIME FROM INDOOR_RANKING_LAPTIME A;''')
	...
	dbCursor.execute('''CREATE TABLE CKC_BI_INDOOR AS
		...
		FROM 
			TEMP_INDOOR_RANKING_PODIUM P,TEMP_INDOOR_RANKING_LAPTIME T
		WHERE
	...
	dbCursor.execute('''CREATE TEMPORARY TABLE IF NOT EXISTS TEMP_PAROLIN_RANKING_LAPTIME AS SELECT *,(SELECT COUNT(*) FROM PAROLIN_RANKING_LAPTIME B WHERE A.BEST_LAP>=B.BEST_LAP) AS RANK_LAPTIME FROM PAROLIN_RANKING_LAPTIME A;''')
	...
	dbCursor.execute('''CREATE TABLE CKC_BI_PAROLIN AS
		SELECT P.kartNumber
			...
			,0.0125 * (P.RANK_PODIUM + T.RANK_LAPTIME) AS RANK
		FROM 
			TEMP_PAROLIN_RANKING_PODIUM P,TEMP_PAROLIN_RANKING_LAPTIME T
		WHERE
			P.kartNumber=T.kartNumber
		GROUP BY 
			P.kartNumber
